...
WINDOW_TITLE_SUBSTR = "Endless Online"   # window title substring to focus

TRIGGER_PHRASE = "reward"
TRIGGER_CHECK_DELAY = 0.20
...
TYPE_INTERVAL = 0.40  # seconds between letters (400ms delay between key presses)

# Avoid resizing the game window when focusing (do not restore/minimize/maximize)
AVOID_RESTORE_ON_FOCUS = True
...
POST_SOLVE_COOLDOWN = 1.0  # seconds to wait after typing solution
# ───────────────────────────────────

# Stability-based OCR aggregation (favor letters that persist across changes)
STABILITY_MIN_OBSERVATIONS = 8
STABILITY_MIN_STREAK = 4
...
def scan_region(region, frames=FRAMES, delay=DELAY):
    ...
    with mss.mss() as sct:
        for i in range(frames):
            ...
            if letter:
                ...
                # Update streaks to prefer persistent letters
                if letter == current_streak_letter:
                    current_streak_len += 1
                else:
                    current_streak_letter = letter
                    current_streak_len = 1
                if current_streak_len > max_streak[letter]:
                    max_streak[letter] = current_streak_len
    ...
    # Choose by longest consecutive streak; tie-break by total count
    def score(l):
        return (max_streak[l], counts[l])
    best = max(max_streak.keys(), key=lambda l: score(l)) if max_streak else max(counts.keys(), key=lambda l: counts[l])

    # Optional stability thresholds: if best is weak, fall back to majority
    if max_streak[best] < STABILITY_MIN_STREAK or counts[best] < STABILITY_MIN_OBSERVATIONS:
        best = counts.most_common(1)[0][0]

    print(f"Answer is {best} (streak={max_streak[best]}, seen={counts[best]})")
    return best
...
def _bring_to_foreground(hwnd):
    ...
    # Optionally avoid any restore that could change window size
    if win32gui.IsIconic(hwnd):
        if AVOID_RESTORE_ON_FOCUS:
            # Do not restore a minimized window; let the caller decide
            return False
        else:
...
def focus_endless_online_and_click_center():
    ...
    if hwnd is None:
        try:
            ...
            if wins:
                w=wins[0]
                # Avoid restoring if configured to prevent size changes
                if w.isMinimized:
                    if AVOID_RESTORE_ON_FOCUS:
                        return False
                    else:
                        w.restore(); time.sleep(0.1)
...
def _title_matches_endless(title):
    t = (title or "").lower()
    candidates = [WINDOW_TITLE_SUBSTR.lower(), "endless", "endless online", "endlessonline", "endless.exe"]
...
def solve_one_captcha(trigger_region, regions, captcha_num):
    ...
    print("\n🔍 Scanning all spots in parallel...")
    with ThreadPoolExecutor(max_workers=min(SPOTS_REQUIRED, 5)) as executor:
    ...
    # Ensure focus is on EndlessOnline before typing (retry a few times)
    focused_ok = False
    for _ in range(5):
        if focus_endless_online_and_click_center():
            ...
            if win32gui:
                ...
                if _title_matches_endless(title):
                    focused_ok = True
                    break
                else:
                    print(f"⚠️ Focus check: current window title is '{title}' (not matching known Endless variants)")
            else:
                # Fallback path (pyautogui) — assume focus succeeded
                focused_ok = True
                break
        time.sleep(0.2)
